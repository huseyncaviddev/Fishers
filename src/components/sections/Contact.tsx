"use client";

import { motion, useInView } from "framer-motion";
import { useRef } from "react";

const CONTACT_INFO = [
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-6 h-6">
        <path d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
        <path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
      </svg>
    ),
    title: "Ünvan",
    lines: ["Bakı şəhəri, Azərbaycan", "Nərimanov rayonu"],
  },
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-6 h-6">
        <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
      </svg>
    ),
    title: "Telefon",
    lines: ["+994 12 345 67 89", "+994 50 123 45 67"],
  },
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-6 h-6">
        <path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
      </svg>
    ),
    title: "E-poçt",
    lines: ["info@fishers.az", "sales@fishers.az"],
  },
];

export function Contact() {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, margin: "-100px" });

  return (
    <section id="contact" className="py-24 lg:py-32 bg-white" ref={ref}>
      <div className="mx-auto max-w-7xl px-6 lg:px-12">
        <motion.div
          initial={{ opacity: 0, y: 30 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.7 }}
          className="text-center max-w-3xl mx-auto mb-16"
        >
          <span className="text-ocean font-medium text-sm tracking-widest uppercase">
            Əlaqə
          </span>
          <h2 className="mt-4 font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-navy">
            Bizimlə <span className="text-ocean">Əlaqə</span> Saxlayın
          </h2>
          <p className="mt-6 text-slate/70 text-lg">
            Suallarınız, təklifləriniz və ya əməkdaşlıq imkanları üçün bizimlə əlaqə saxlayın.
          </p>
        </motion.div>

        <div className="grid lg:grid-cols-5 gap-12">
          <motion.div
            initial={{ opacity: 0, x: -30 }}
            animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.7, delay: 0.2 }}
            className="lg:col-span-2 space-y-8"
          >
            {CONTACT_INFO.map((item, i) => (
              <div key={i} className="flex gap-4">
                <div className="w-12 h-12 rounded-xl bg-ocean-light text-ocean flex items-center justify-center shrink-0">
                  {item.icon}
                </div>
                <div>
                  <h4 className="font-semibold text-navy">{item.title}</h4>
                  {item.lines.map((line, j) => (
                    <p key={j} className="text-slate/70 mt-1">
                      {line}
                    </p>
                  ))}
                </div>
              </div>
            ))}

            <div className="pt-6 border-t border-ocean-muted/30">
              <h4 className="font-semibold text-navy mb-4">Bizi İzləyin</h4>
              <div className="flex gap-3">
                {["facebook", "instagram", "linkedin"].map((social) => (
                  <a
                    key={social}
                    href="#"
                    className="w-10 h-10 rounded-full bg-ocean-light text-ocean flex items-center justify-center hover:bg-ocean hover:text-white transition-all duration-300"
                    aria-label={social}
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                      {social === "facebook" && (
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                      )}
                      {social === "instagram" && (
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                      )}
                      {social === "linkedin" && (
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                      )}
                    </svg>
                  </a>
                ))}
              </div>
            </div>
          </motion.div>

          <motion.form
            initial={{ opacity: 0, x: 30 }}
            animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ duration: 0.7, delay: 0.3 }}
            className="lg:col-span-3 bg-mist rounded-2xl p-8 lg:p-10"
            onSubmit={(e) => e.preventDefault()}
          >
            <div className="grid sm:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-medium text-navy mb-2">
                  Ad Soyad
                </label>
                <input
                  type="text"
                  placeholder="Adınız"
                  className="w-full px-4 py-3 rounded-xl border border-ocean-muted/30 bg-white text-navy placeholder:text-slate/40 focus:outline-none focus:ring-2 focus:ring-ocean/30 focus:border-ocean transition-all"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-navy mb-2">
                  E-poçt
                </label>
                <input
                  type="email"
                  placeholder="email@example.com"
                  className="w-full px-4 py-3 rounded-xl border border-ocean-muted/30 bg-white text-navy placeholder:text-slate/40 focus:outline-none focus:ring-2 focus:ring-ocean/30 focus:border-ocean transition-all"
                />
              </div>
            </div>
            <div className="mt-5">
              <label className="block text-sm font-medium text-navy mb-2">
                Mövzu
              </label>
              <input
                type="text"
                placeholder="Mesajınızın mövzusu"
                className="w-full px-4 py-3 rounded-xl border border-ocean-muted/30 bg-white text-navy placeholder:text-slate/40 focus:outline-none focus:ring-2 focus:ring-ocean/30 focus:border-ocean transition-all"
              />
            </div>
            <div className="mt-5">
              <label className="block text-sm font-medium text-navy mb-2">
                Mesaj
              </label>
              <textarea
                rows={5}
                placeholder="Mesajınızı buraya yazın..."
                className="w-full px-4 py-3 rounded-xl border border-ocean-muted/30 bg-white text-navy placeholder:text-slate/40 focus:outline-none focus:ring-2 focus:ring-ocean/30 focus:border-ocean transition-all resize-none"
              />
            </div>
            <button
              type="submit"
              className="mt-6 w-full sm:w-auto px-8 py-3.5 bg-ocean text-white font-medium rounded-full hover:bg-ocean-dark transition-all duration-300 hover:shadow-lg hover:shadow-ocean/30"
            >
              Göndər
            </button>
          </motion.form>
        </div>
      </div>
    </section>
  );
}
